...
public class CoverFlowView extends RelativeLayout {
    ...
    private void makeChildTransformer(View child, int position, float offset) {
        ...
        // 278
        final int originalChildHeight = (int) (mChildHeight - mChildHeight * reflectHeightFraction - reflectGap);
...
//        }
        ...
        if (offset <= 0) {
            ...
            translateX = mWidth - ((float) rightSpace / VISIBLE_VIEWS) * (VISIBLE_VIEWS - offset) - childWidth - paddingRight;
        }
        // 0 105 510 360 210 0 105
        // 根据offset 算出透明度
...
//        mVelocity.recycle();
    ...
    // 是否正在进行点击切换动画
    private boolean isOnAnimator = false;
