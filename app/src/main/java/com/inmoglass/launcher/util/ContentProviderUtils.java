...
public class ContentProviderUtils {
    ...
    public static void insertMemoRecords() {
        InmoMemoData needInsertData = new InmoMemoData();
        needInsertData.setContent("我是一条测试数据");
        needInsertData.setTimestamp(1645697116419L);
        needInsertData.setCreateTime(1645697116419L);
        LogUtils.d(TAG, "是否重复数据 = " + checkIsRepeatData(needInsertData));
        if (!checkIsRepeatData(needInsertData)) {
            insertToMemoContentProvider(needInsertData);
        }
    }
