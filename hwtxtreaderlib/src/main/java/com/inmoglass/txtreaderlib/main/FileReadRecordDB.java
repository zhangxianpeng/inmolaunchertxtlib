...
//                    + "','" + path + "','" + fileName + "','" + paragraphIndex + "','" + chartIndex + "')";
            //防止恶意输入
            String sql = " insert into " + TABLE_NAME + " (" + FileHashName + "," + FilePath + "," + FileName + "," + ParagraphIndex + "," + ChartIndex + ") values (?,?,?,?,?)";

            Object[] fileinfo = new Object[]{fileHashName, path, fileName, paragraphIndex, chartIndex};
            getWritableDatabase().execSQL(sql,   fileinfo);
        }
    }
